...
# ---------------------------------------------------------------------------
# Interview Tools (State-Safe for LiveKit)
# ---------------------------------------------------------------------------

# Note: session_id is now passed explicitly to avoid async concurrency bugs.
...
def submit_rubric_grade(session_id: str, category: str, grade: str, notes: str) -> dict[str, str]:
    ...
    val_grade = grade.lower().strip()
    
    if val_grade not in _VALID_GRADES:
        return {"error": f"Invalid grade '{val_grade}'. Must be one of: {', '.join(sorted(_VALID_GRADES))}"}
        
    state.grades[cat_key] = {"grade": val_grade, "notes": notes}
    logger.info(f"[rubric] {cat_key}: {val_grade} — {notes}")
    return {"status": "graded", "category": cat_key, "grade": val_grade}
...
def get_rubric_report(session_id: str, scope: str = "current") -> dict[str, Any]:
    ...
    lines = [f"{cat}: {data['grade'].upper()} — {data['notes']}" for cat, data in selected.items()]
    return {
        "report": "\n".join(lines),
...
def get_round_scorecard(session_id: str, round_number: int = 0, category: str = "", scope: str = "current") -> dict[str, Any]:
    state = _get_state(session_id)
    delta = get_session_delta(session_id)
    scope_key = (scope or "current").strip().lower()
    
    grade_source = state.grades if scope_key == "overall" else delta["grades"]
    scorecard = _build_round_scorecard(state, round_number=round_number, category=category, grades=grade_source)
...
def end_conversation(session_id: str) -> dict[str, Any]:
    logger.info(f"[interview] end_conversation requested by LLM for {session_id}")
...
def evaluate_candidate_answer(session_id: str, question: str, strength: str, weakness: str, category_grades: list[dict[str, str]] | None = None) -> dict[str, Any]:
    note_result = record_answer_note(session_id, question, strength, weakness)
    grade_results = []
    ...
    if category_grades:
        for g in category_grades:
            ...
            if "category" in normalized and "grade" in normalized and "notes" in normalized:
                res = submit_rubric_grade(session_id, normalized["category"], normalized["grade"], normalized["notes"])
...
def get_session_summary(session_id: str, scope: str = "overall") -> dict[str, str]:
    ...
    if count == 0:
        message = "No new questions have been asked yet." if scope_key == "current" else "No questions have been asked yet."
        result = {"summary": message}
    else:
        summary = f"{count} question{'s' if count != 1 else ''} covered: " + "; ".join(q[:50] + "…" for q in questions)
    ...
    if scope_key != "current" and current_count:
        ...
        result["current_session_summary"] = (
            f"{current_count} questions covered in this live session: " + "; ".join(q[:50] + "…" for q in delta["questions"])
        )
    return result